    ...
    <div class="card-body">
        ...
        <form action="<?= base_url('admin/prefixes/ajouter') ?>" method="post" class="row g-3 align-items-end">
            ...
            <div class="col-auto">
                ...
                <input type="text" class="form-control" id="operateur" name="operateur" placeholder="ex: Orange">
            </div>
            <div class="col-auto">
                <div class="form-check mt-4">
                    <input class="form-check-input" type="checkbox" id="est_operateur_propre" name="est_operateur_propre" value="1" checked>
                    <label class="form-check-label" for="est_operateur_propre">Telma (notre opérateur)</label>
                </div>
            </div>
            <div class="col-auto">
        ...
        <p class="text-muted mt-2">Décochez « Telma (notre opérateur) » pour déclarer un préfixe appartenant à un autre opérateur (transferts inter-opérateurs).</p>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                <thead>
                    <tr><th>Préfixe</th><th>Opérateur</th><th>Statut</th><th>Actions</th></tr>
                </thead>
                <tbody>
                <?php foreach ($prefixes as $p): ?>
                    <tr>
                        <td><?= esc($p['prefixe']) ?></td>
                        <td>
                            <?= esc($p['operateur'] ?? 'Telma') ?>
                            <?php if (empty($p['est_operateur_propre'])): ?>
